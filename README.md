...
- **📹 多格式链接解析**：支持完整URL、短链接、直接BV/AV号自动识别
- **🌐 双语处理模式**：中文模式（默认）/ 英文双语模式（添加 `#en` 参数）⭐
- **⚡ 智能下载策略**：优先yt-dlp音频流（1-2秒），失败自动回退you-get，支持断点续传
- **🎤 Whisper语音识别**：Base模型，支持中英文识别及自动语言检测
- **✨ DeepSeek AI增强**：原文精转（修正错别字、标点、表达）+ 关键纪要（结构化摘要），移除输出限制充分利用API能力
- **📝 上下文理解**：视频标题注入提示词，提升专业术语处理准确性
- **📄 飞书云文档**：自动创建Markdown文档，生成分享链接，支持原文精转和关键纪要两个文档
- **💾 智能缓存系统**：按视频ID和语言分别缓存，支持快速重建，防重复下载，并发保护
