...
### ⚡ 核心特性
- **智能下载**：优先yt-dlp音频流（1-2秒），失败自动回退you-get，支持断点续传
- **Whisper识别**：Base模型，支持中英文识别及自动语言检测
- **DeepSeek增强**：原文精转（修正错别字、标点、表达）+ 关键纪要（结构化摘要），移除输出限制充分利用API能力
- **上下文理解**：视频标题注入提示词，提升专业术语处理准确性
- **飞书云文档**：自动创建Markdown文档，生成分享链接，支持原文精转和关键纪要两个文档
- **智能缓存**：按视频ID和语言分别缓存，支持快速重建，防重复下载，并发保护
