...
```mermaid
graph TB
    User[👤 用户] -->|发送视频链接| Feishu[飞书]

    subgraph 飞书服务
        Feishu -->|WebSocket长连接| WSClient[FeishuWSClient]
        WSClient -->|事件分发| Handler[FeishuHandler]
    end

    subgraph 核心处理流程
        Handler -->|提取视频ID+语言| Task[TaskProcessor]

        Task -->|检查| Cache[缓存系统]
        Cache -->|命中| DocGen[文档生成]
        Cache -->|未命中| Downloader

        subgraph 下载模块
            Downloader[视频下载器]
            Downloader -->|优先| YTDLP[yt-dlp<br/>音频下载]
            Downloader -->|回退| YouGet[you-get<br/>视频下载]
            YouGet -->|提取音频| Audio[AudioUtils]
        end

        subgraph 识别处理
            YTDLP -->|Whisper ASR| ASR[Whisper语音识别]
            Audio -->|Whisper ASR| ASR
        end

        subgraph AI处理
            ASR -->|视频标题+文本| LLM[DeepSeek API]
            LLM -->|原文精转| Refined[精转文本]
            LLM -->|关键纪要| Summary[纪要文本]
        end
    ...
    subgraph 文档生成
        Cache -->|创建文档| FeishuDoc[飞书文档API]
        FeishuDoc -->|分享链接| User
        DocGen -->|创建文档| FeishuDoc
    end

    subgraph 辅助模块
        Tracker[ProcessingTracker<br/>处理状态追踪]
    ...
    style Feishu fill:#fff4e6
    style WSClient fill:#f3f9ff
    style Handler fill:#f3f9ff
    style Task fill:#fff0f6
    style Cache fill:#fff9c4
    style Downloader fill:#e8f5e9
    style YTDLP fill:#c8e6c9
    style YouGet fill:#c8e6c9
    style ASR fill:#b3e5fc
    style LLM fill:#ffcc80
    style FeishuDoc fill:#fff4e6
...
3. **核心处理层**
   - **缓存检查**：优先使用缓存结果（秒级响应）
   - **音频下载**：yt-dlp优先（1-2秒），you-get回退
   - **语音识别**：Whisper Base模型（30秒-1分钟）
   - **AI处理**：DeepSeek API精转和摘要（2-5分钟）
...
| 层级 | 技术栈 | 用途 |
|------|--------|------|
| **通信层** | lark-oapi WebSocket | 飞书长连接 |
| **下载层** | yt-dlp / you-get | 音频/视频下载 |
| **识别层** | OpenAI Whisper | 语音识别 |
| **AI层** | DeepSeek API | 文本精转和摘要 |
| **文档层** | lark-oapi Doc API | 飞书文档创建 |
| **缓存层** | JSON文件 | 本地缓存 |
